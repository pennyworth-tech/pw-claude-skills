#!/bin/bash
# Web Component Testing Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="web-component-testing"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("component" "testing")
...
[[ "$skill_name" != "web-component-testing" ]] && { error "Invalid skillName"; exit 1; }

# Check testResults exists
results=$(json_get "$OUTPUT_FILE" ".output.testResults")
[[ -z "$results" || "$results" == "null" ]] && { error "Missing testResults in output"; exit 1; }
