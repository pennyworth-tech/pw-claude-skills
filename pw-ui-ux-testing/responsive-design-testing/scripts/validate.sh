#!/bin/bash
# Responsive Design Testing Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="responsive-design-testing"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("responsive" "viewport")
...
[[ "$skill_name" != "responsive-design-testing" ]] && { error "Invalid skillName"; exit 1; }

# Check viewportResults exist
viewports=$(json_get "$OUTPUT_FILE" ".output.viewportResults")
[[ -z "$viewports" || "$viewports" == "null" ]] && { error "Missing viewportResults in output"; exit 1; }
