#!/bin/bash
# iOS Visual Testing Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="ios-visual-testing"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("snapshot" "visual")
...
[[ "$skill_name" != "ios-visual-testing" ]] && { error "Invalid skillName"; exit 1; }

# Check snapshotResults exists
snapshots=$(json_get "$OUTPUT_FILE" ".output.snapshotResults")
[[ -z "$snapshots" || "$snapshots" == "null" ]] && { error "Missing snapshotResults in output"; exit 1; }
