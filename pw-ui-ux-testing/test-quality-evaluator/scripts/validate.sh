#!/bin/bash
# Test Quality Evaluator Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="test-quality-evaluator"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("qualityGates" "evaluations" "summary")
...
if [[ "$skill_name" != "test-quality-evaluator" ]]; then
  error "Invalid skillName: expected 'test-quality-evaluator', got '$skill_name'"
  exit 1
fi
success "skillName correct"

# Step 4: Check output.summary exists
average_score=$(json_get "$OUTPUT_FILE" ".output.summary.averageScore")
if [[ -z "$average_score" || "$average_score" == "null" ]]; then
  error "Missing output.summary.averageScore"
  exit 1
fi
success "summary.averageScore present: $average_score"
...
# Step 6: Check evaluations array exists
eval_count=$(json_count "$OUTPUT_FILE" ".output.evaluations")
if [[ -z "$eval_count" || "$eval_count" == "null" ]]; then
  eval_count=0
fi
success "Evaluations array present: $eval_count files"
...
# Step 8: Check methodology honesty note
note=$(json_get "$OUTPUT_FILE" ".output.summary.note")
if [[ -n "$note" && "$note" != "null" ]]; then
  success "Methodology note present (honest about scoring method)"
else
  warn "Missing summary.note -- should document scoring methodology"
fi
