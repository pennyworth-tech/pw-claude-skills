#!/bin/bash
# iOS Performance Testing Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="ios-performance-testing"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("performance" "iOS")
...
[[ "$skill_name" != "ios-performance-testing" ]] && { error "Invalid skillName"; exit 1; }

# Check performanceMetrics exists
metrics=$(json_get "$OUTPUT_FILE" ".output.performanceMetrics")
[[ -z "$metrics" || "$metrics" == "null" ]] && { error "Missing performanceMetrics in output"; exit 1; }
