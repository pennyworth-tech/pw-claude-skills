#!/bin/bash
# Web Performance Testing Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="web-performance-testing"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("performance" "LCP" "CLS")
...
[[ "$skill_name" != "web-performance-testing" ]] && { error "Invalid skillName"; exit 1; }

# Check Core Web Vitals exist
cwv=$(json_get "$OUTPUT_FILE" ".output.coreWebVitals")
[[ -z "$cwv" || "$cwv" == "null" ]] && { error "Missing coreWebVitals in output"; exit 1; }
