#!/bin/bash
# Test Inventory Skill Validator v1.0.0
set -euo pipefail
...
SKILL_NAME="test-inventory"
SKILL_VERSION="1.0.0"
SCHEMA_PATH="$SKILL_DIR/schemas/output.schema.json"
MUST_CONTAIN_TERMS=("qualityGates" "services" "summary")
...
# Step 2: Schema validation (non-blocking -- may skip if no validator)
...
if [[ "$skill_name" != "test-inventory" ]]; then
  error "Invalid skillName: expected 'test-inventory', got '$skill_name'"
  exit 1
fi
success "skillName correct"

# Step 4: Check output.summary exists
total_test_files=$(json_get "$OUTPUT_FILE" ".output.summary.totalTestFiles")
if [[ -z "$total_test_files" || "$total_test_files" == "null" ]]; then
  error "Missing output.summary.totalTestFiles"
  exit 1
fi
success "summary.totalTestFiles present: $total_test_files"
...
# Step 6: Check services array exists
services_count=$(json_count "$OUTPUT_FILE" ".output.services")
if [[ -z "$services_count" || "$services_count" == "null" ]]; then
  services_count=0
fi
success "Services array present: $services_count services"

# Step 7: Check analysisType is static
analysis_type=$(json_get "$OUTPUT_FILE" ".output.analysisType")
if [[ "$analysis_type" != "static" ]]; then
  error "Invalid analysisType: expected 'static', got '$analysis_type'"
  exit 1
fi
success "analysisType is 'static'"

# Step 8: Check coverage note honesty
coverage_note=$(json_get "$OUTPUT_FILE" ".output.summary.coverage.note")
if [[ -n "$coverage_note" && "$coverage_note" != "null" ]]; then
  success "Coverage note present (honest about estimation method)"
else
  warn "Missing coverage.note -- should document estimation method"
fi
