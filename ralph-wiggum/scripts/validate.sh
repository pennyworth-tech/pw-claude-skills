#!/bin/bash
# Ralph Wiggum Skill Validator v1.0.0
set -euo pipefail
...
MUST_CONTAIN_TERMS=("iteration" "loop")
...
[[ "$skill_name" != "ralph-wiggum" ]] && { error "Invalid skillName"; exit 1; }

# Check iterationResults exist
iterations=$(json_get "$OUTPUT_FILE" ".output.iterationResults")
[[ -z "$iterations" || "$iterations" == "null" ]] && { error "Missing iterationResults in output"; exit 1; }

# Check completionStatus
completion=$(json_get "$OUTPUT_FILE" ".output.iterationResults.completionStatus")
[[ -z "$completion" || "$completion" == "null" ]] && { error "Missing completionStatus"; exit 1; }
